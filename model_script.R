...
# Create df of Predicted values, xkm and ykm are in utm29
# Locp - A matrix
# p.pred - P vector
...
pred_studyarea_df <- make_prediction_map(Locp, Mean_response)
head(pred_studyarea_df)

#_______________________________________________________________________________

# 2050 rcp45 prediction stack
pred <- inla.stack(data = list(y = NA), 
                   A = list(Aprediction, 1),                 
                   effects = list(w=1:spde$n.spde,
                                  list(Intercept = 1,
                                       dcoast.std      = pp_studyarea$dcoast.std,
                                       current.std   = pp_studyarea$current45.std,
                                       temp.std      = pp_studyarea$temp45.std
                                  )),
                   tag = 'pred')
...
# Rerun the final model with the empty prediction stack to generate the response variable.
pred_model<-inla(final, data=inla.stack.data(stk, spde=spde),
                 family= 'binomial', quantiles = NULL,
                 control.predictor=list(link = 1, A=inla.stack.A(stk),compute=FALSE),  
                 control.compute = list(config = TRUE), 
                 control.inla(strategy = 'simplified.laplace', huge = TRUE),
...
# Extracting Predicted Values
index.pred<-inla.stack.index(stk, "pred")$data
post.mean.pred.logit<-pred_model$summary.linear.predictor[index.pred,"mean"]
p.pred<-exp(post.mean.pred.logit)/(1 + exp(post.mean.pred.logit))


# Rename vector of Predicted values 
Mean_response_45 <- p.pred

# Append to existing df

pred_studyarea_df <- append_prediction_map(pred_studyarea_df, Mean_response_45)
head(pred_studyarea_df)

#_______________________________________________________________________________

# 2050 rcp85 prediction stack
pred <- inla.stack(data = list(y = NA), 
                   A = list(Aprediction, 1),                 
                   effects = list(w=1:spde$n.spde,
                                  list(Intercept = 1,
                                       dcoast.std      = pp_studyarea$dcoast.std,
                                       current.std   = pp_studyarea$current85.std,
                                       temp.std      = pp_studyarea$temp85.std
                                  )),
                   tag = 'pred')
...
# Rename vector of Predicted values 
Mean_response_85 <- p.pred

# Append to existing df

pred_studyarea_df <- append_prediction_map(pred_studyarea_df, Mean_response_85, remove_obs = T)
head(pred_studyarea_df)

write.csv(pred_studyarea_df,'export/response_studyarea.csv')
